...
function SignUp({ onSignUpOpen, onSubmit }) {
  ...
  const handleSubmit = (event) => {
    ...
    if (password == confirmPassword) {
      ...
      const signup = new CustomEvent("onSignUp", {
        bubbles: true,
        cancelable: true,
        detail: masterData
      });
      document.dispatchEvent(signup);
      // Event dispatch for Db update;
    }else{
  ...
  return (
    <>
      <div
        ...
        className="relative flex z-50 max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
      >
